...
def robotControl(trigger, simultaneous_launcher, motor_command_queue, sensor_queue):
    ...
    while not (trigger.is_set() or simultaneous_launcher.broken or gopigo3_robot.volt() <= MINIMUM_VOLTAGE):
        ...
        if move is False or turnCount == 4:
            gopigo3_robot.stop()
            
        else:
            with open('problem2_pathtrace.csv', mode='w') as csv_file:
                fieldnames = ['Row', 'Encoder', 'Distance']
                writer = csv.DictWriter(csv_file, fieldnames=fieldnames)

                writer.writeheader()

                gopigo3_robot.drive_cm(50);
                print("Encoder L: %6d  R: %6d" % (gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT), gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)))
                print("Distance reading: " + str(my_distance_sensor.read_mm()))
                writer.writerow({'Row': '1', 'Encoder': str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT)) + ' , ' + str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)) , 'Distance': str(my_distance_sensor.read_mm())})

                gopigo3_robot.turn_degrees(90, blocking = True)
                gopigo3_robot.drive_cm(50);
                print("Encoder L: %6d  R: %6d" % (gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT), gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)))
                print("Distance reading: " + str(my_distance_sensor.read_mm()))
                writer.writerow({'Row': '2', 'Encoder': str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT)) + ' , ' + str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)) , 'Distance': str(my_distance_sensor.read_mm())})

                gopigo3_robot.turn_degrees(90, blocking = True)
                gopigo3_robot.drive_cm(50);
                print("Encoder L: %6d  R: %6d" % (gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT), gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)))
                print("Distance reading: " + str(my_distance_sensor.read_mm()))
                writer.writerow({'Row': '3', 'Encoder': str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT)) + ' , ' + str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)) , 'Distance': str(my_distance_sensor.read_mm())})

                gopigo3_robot.turn_degrees(90, blocking = True)
                gopigo3_robot.drive_cm(50);
                print("Encoder L: %6d  R: %6d" % (gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT), gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)))
                print("Distance reading: " + str(my_distance_sensor.read_mm()))
                writer.writerow({'Row': '4', 'Encoder': str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_LEFT)) + ' , ' + str(gopigo3_robot.get_motor_encoder(gopigo3_robot.MOTOR_RIGHT)) , 'Distance': str(my_distance_sensor.read_mm())})
