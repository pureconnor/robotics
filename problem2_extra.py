...
def robotControl(trigger, simultaneous_launcher, motor_command_queue, sensor_queue):
    ...
    while not (trigger.is_set() or simultaneous_launcher.broken or gopigo3_robot.volt() <= MINIMUM_VOLTAGE):
        ...
        if direction_degrees is not None and move is False:
        ...
        if move is False:
            ...
            with picamera.PiCamera() as camera:
                print("Picture captured. Deciding where to go...")
                camera.resolution = (640, 480)
                camera.rotation = 180
                camera.capture(output, format = 'rgb', use_video_port = True)
                img = Image.fromarray(output)
                img.save("/home/jupyter/Module2/Assignments/Problem2_photos/module2_problem2_"+str(photo_count)+".jpg") 
                photo_count += 1
            ...
            servo1.rotate_servo(servo1_position)
            sleep(1.250)
            left = my_distance_sensor.read_mm()
            print("To the left there is an object " + str(left) + "mm away")
            sleep(1.000)
            ...
            servo1.rotate_servo(servo1_position)
            sleep(1.250)
            right = my_distance_sensor.read_mm()
            print("To the right there is an object " + str(right) + "mm away")
            sleep(1.000)
            
            #Move in direction with less obstruction, left by default
            if(left < right):
                print("Turning right...")
                ...
                gopigo3_robot.turn_degrees(180, blocking = True)
