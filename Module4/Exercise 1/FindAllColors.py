...
if __name__ == "__main__":  
    ...
    lock = threading.Lock()
    #self. = np.array([0, 255, 0]) # green
    #border_color = np.array([255, 0, 0]) # red
    color_threshold = 0.10 # in percentage
    time_to_run = 10 # in seconds

    start = time.time()
    imageThreadRed = ImageProcessor(thread_stopper, frames, lock, np.array([255,0,0]), np.array([0, 0, 255])) #find red with blue border
    imageThreadGreen = ImageProcessor(thread_stopper, frames, lock, np.array([0,255,0]), np.array([255, 0, 0])) #find green with red border
    imageThreadBlue = ImageProcessor(thread_stopper, frames, lock, np.array([0,0,255]), np.array([0, 255, 0])) #find blue with green border
